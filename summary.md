...
|特征|解释|
|:-------------:|:-----:|
|user_type#n|对店家使用DBSCAN进行聚类后进行onehot编码|
...
我们想起了信号处理里面的中值滤波,
索性直接将PS-SMART及GBDT所得到的4个预测结果取中值。
以期为每个店自动选择合适的模型进行预测，
并剔除不合适的异常模型对结果造成的影响。
SQL代码是这样的:
```sql
...
所以最后一天提交结果基本上用了中值结果，但考虑到保险起见，毕竟只有最后一次机会了，仍然加权了ARIMA及线性回归融合模型结果，现在看来不加权可能会更好一点。考虑到31日为元旦假期首日，14日(哎.. 27号忘记看了)最低温度过低，加入了人工规则进行修正。

## 关于人工调整
在上述模型预测中，我们对模型训练结果进行过手动的微调。
但是这些个微调是不是合理的呢？这就要靠撞大运了。

起初在线下时，我们常将模型融合完的计算结果乘以一个尺度系数来得到预测结果。
...
在最后一次的评测中，考虑到温度和假期因素，也加入了人工规则进行修正。
然而这次就没这么幸运啦。
...
#### 企业聚类
在这次比赛中我们没有找到好的聚类方式，
在线下的时候尝试过对企业用电情况的一个聚类，
由于当时只是单纯的对电量数据使用DBSCAN划分，
聚类的效果并不好。
然后因为懒，
不想一家店一家店去看(说好听点是这样的工作量在实际应用场景中是不现实的)，
所以最后线上的比赛中并没有对企业分类。
但是后来想了一下，
其实可以利用温度和节假日等特征进行聚类，
或者使用相关性分析的方法筛选出不同的企业，
在保证每种企业训练样本充足的情况下，
使用不同的模型进行训练，
或者将分类好的分类信息作为onehot变量加入之前的模型。
如果分好类的话说不定还能有很大的提升。

另一方面，我们要转变下思路。
我们组总是认为构造一堆模型，然后用层叠或者组合的方式可以自动找到商店所匹配的模型，妄想不分类搞定商店。但是实际上，这么做是不太靠谱的。

#### onehot
还有一个问题是到底应不应该做onehot？
onehot编码特征(如假期等)太过稀疏了，直接拿来用tree based model训练，在节点分裂的时候不一定会被看上。
所以准备在下次比赛时试试对onehot编码后结果先过个线性回归试试会不会好一点。大致思路是如下图
